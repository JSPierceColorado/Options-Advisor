...
def _get(path, params=None):
    ...
    try:
        ...
        data = {}
    time.sleep(S.API_DELAY)
    return data

def _post(path, data):
    url = S.TRADIER_BASE.rstrip("/") + path
    try:
        r = requests.post(url, headers={**_hdr(), "Content-Type": "application/x-www-form-urlencoded"}, data=data, timeout=25)
        data = r.json()
    except Exception as e:
...
# -------------------- Tradier helpers --------------------
def quote(sym):
    d = _get("/v1/markets/quotes", {"symbols": sym})
    return (d.get("quotes") or {}).get("quote")
...
def order(sym, side, qty, otype="limit", price=None):
    if S.DRY_RUN:
        return {"status": "dry_run", "sym": sym, "side": side, "qty": qty, "price": price}
    if not S.TRADIER_ACCOUNT:
...
def pick_call(t, e, today):
    ...
    df["spr"] = (df["ask"] - df["bid"]) / df["mid"].clip(1e-9)
    df = df[(df.open_interest >= S.OI_MIN) & (df.volume >= S.VOL_MIN) & (df.spr <= S.MAX_SPREAD) &
            df.delta.between(S.DELTA_MIN, S.DELTA_MAX)]
    if df.empty:
        return None
    target = 0.5 * (S.DELTA_MIN + S.DELTA_MAX)
    df["drank"] = (df["delta"] - target).abs()
...
# -------------------- Core --------------------
def manage_open(t, spot, atrv, cnt):
    for p in [x for x in db_all() if x["ticker"] == t]:
        hi = max(p["highest_close"], spot)
        trail = chandelier(hi, atrv)
        q = quote(p["contract"]) or {}
        ...
        if spot < trail:
            ...
            if (exp.date() - datetime.now(UTC).date()).days <= S.DTE_STOP:
                reason = "time"
        if reason:
...
def process(t, cnt):
    ...
    for e in exps:
        ...
        if c:
            ...
            db_add((t, c.contract, c.expiry, c.strike, c.delta, spot, c.mid, spot, chandelier(spot, a),
...
def run_once():
    ...
    db_init()
    syms = universe()
    print(f"[INFO] universe {len(syms)} tickers")
