# ==========================================================
# OptionExecutor v2.4 — S&P 500 (CSV) → Alpaca Screener → Tradier Options
# - S&P 500 list from CSV mirrors (no lxml). Small fallback if mirrors fail.
# - Screener: tighter trend filter to avoid overbought/extended names.
...
#          plus existing ATR chandelier, peak drawdown, and DTE-based time exit.
...
class S:
    ...
    # --- Exits (NEW) ---
    TAKE_PROFIT = _f("OPT_TAKE_PROFIT", 0.25)   # +25% TP on option premium
    STOP_LOSS   = _f("OPT_STOP_LOSS", 0.08)     # -8% SL on option premium
...
def _alpaca_headers():
    return {
        ...
        "User-Agent": "OptionExecutor/2.4 (+S&P500 screener)"
    }
...
def trend_ok(df):
    """
    Tighter health check to avoid overbought/extended names:
    - EMA20 > EMA50
    - RSI rising and bounded: 50 < RSI(14) < 70
...
def sp500_symbols() -> List[str]:
    ...
    sess.headers.update({"User-Agent": "Mozilla/5.0 OptionExecutor/2.4"})
...
def manage_open_for_ticker(t: str, cnt: Dict[str,int]):
    ...
    for p in picks:
        ...
        # Existing: ATR chandelier on underlying
        elif atr_val is not None and spot < trail:
            reason = "trail"
...
def screen_symbol(sym: str) -> bool:
    ...
    if DEBUG:
        if passed:
            ...
            print(f"[PASS] {sym}: price={price:.2f}, EMA{S.EMA_FAST}={ef:.2f}>EMA{S.EMA_SLOW}={es:.2f}, RSI={r_last:.1f}")
        else:
